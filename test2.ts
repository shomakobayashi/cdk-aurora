...
export default function () {
    ...
    // APIエンドポイントを環境変数から取得
    const apiUrl = __ENV.RDS_PROXY_API_URL || __ENV.API_URL;
    
    // 環境変数のデバッグ出力
    if (__ITER === 0) {
        console.log(`TEST_TYPE: ${testType}`);
        console.log(`API URL: ${apiUrl}`);
    }
    
    // URLが設定されていない場合はテストを中止
    if (!apiUrl) {
        fail('API URL is not set. Please provide RDS_PROXY_API_URL or API_URL environment variable.');
        return;
    }
            ...
            'valid response body': (r) => {
                try {
                    ...
                    if (testType === 'READ') {
                        ...
                        return body.user && body.user.id && body.user.name;
                    }
                } catch (e) {
                    if (__ITER === 0) console.log(`Failed to parse response body: ${r.body}`);
                    return false;
                }
            }
        });
